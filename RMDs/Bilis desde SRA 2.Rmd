...
<https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP056183&o=assay_type_s%3Aa%3Bacc_s%3Aa&s=SRR1918902,SRR1920924,SRR1920926,SRR1920929,SRR1920932,SRR1920946,SRR1920953,SRR1920965,SRR1920967,SRR1920969,SRR1920972,SRR1920974,SRR1920980,SRR1920982>
...
# Directorio donde se encuentran los archivos de secuencia
sequence_dir="/ruta/al/directorio/de/archivos"
output_file="manifest.tsv"

# Escribir el encabezado del manifiesto
echo -e "sample-id\tforward-absolute-filepath\treverse-absolute-filepath" > "$output_file"

# Iterar sobre los archivos forward en el directorio
for forward_file in "$sequence_dir"/*_1.fastq; do
    # Obtener el sample-id eliminando la parte '_1'
    sample_id=$(basename "$forward_file" | sed 's/_1.*//')
    
    # Ruta absoluta del archivo forward
    forward_path=$(realpath "$forward_file")
    
    # Obtener la ruta del archivo reverse reemplazando '_1' por '_2'
    reverse_file="${forward_file/_1/_2}"
    reverse_path=$(realpath "$reverse_file")
    
    # Añadir la línea al archivo de manifiesto
    echo -e "$sample_id\t$forward_path\t$reverse_path" >> "$output_file"
done

echo "Manifiesto generado en $output_file"
```
...
biom convert -i exports/14e19092-8aef-4054-9396-bc264b1fbb86/data/feature-table.biom -o exports/14e19092-8aef-4054-9396-bc264b1fbb86/data/feature-table.tsv --to-tsv
...
otutable <- read.table("../Bilis_SRA2/exports/14e19092-8aef-4054-9396-bc264b1fbb86/data/feature-table.tsv", sep = "\t", header = T, skip = 1, comment.char = "")
otutable <- otutable %>% column_to_rownames(var = "X.OTU.ID")

sampleData <- read.table("../Bilis_SRA2/metadata.tsv", sep="\t", header = T)
sampleData <- sampleData %>% dplyr::filter(Sampleid != "SRR1920982") %>% column_to_rownames(var = "Sampleid")


taxonomy <- read.table("../Bilis_SRA2/exports/taxonomy.tsv", sep = "\t", header = T)
...
```{r}
# Create a TreeSE
tse_bilis_sra2 <- TreeSummarizedExperiment(
...
```{r}
saveRDS(tse_bilis_sra2, "../RDSs/tse_bilis_sra2.RDS")
```

...
<https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP227334&o=assay_type_s%3Aa>
...
otutable <- read.table("../Bilis_SRA/exports/f0676277-76f2-4166-b4f5-9d0b5111f0e7/data/feature-table.tsv", sep = "\t", header = T, skip = 1, comment.char = "")
otutable <- otutable %>% column_to_rownames(var = "X.OTU.ID")

sampleData <- read.table("../Bilis_SRA/metadata.tsv", sep="\t", header = T)
sampleData <- sampleData %>% column_to_rownames(var = "Sample.id")


taxonomy <- read.table("../Bilis_SRA/exports/taxonomy.tsv", sep = "\t", header = T)
...
min_n_seqs <- abundances_long %>% 
    dplyr::filter(sample != "SRR10359500") %>%
    group_by(sample) %>%
    summarize(n_seqs = sum(Abundance),
              min = min(n_seqs)) %>%
    pull(min)
...
sampleData <- sampleData[rownames(sampleData) != "SRR10359500", ]
sampleData <- sampleData[rownames(sampleData) != "SRR10359502", ]
...
```{r}
# Create a TreeSE
tse_bilis_sra <- TreeSummarizedExperiment(
...
```{r}
saveRDS(tse_bilis_sra, "../RDSs/tse_bilis_sra.RDS")
```

---
title: "Lectura conteos desde fastq"
...
Lo primero de todo es crear el manifiesto en un archivo csv. Al ser archivos individuales tiene que tener la siguiente cabecera:
- sample-id
- absolute pathway
- direction
...
biom convert -i exports/fbcd37c1-7b15-4145-b71f-956bb3073fc6/data/feature-table.biom -o exports/fbcd37c1-7b15-4145-b71f-956bb3073fc6/data/feature-table.tsv --to-tsv
...
otutable <- read.table("../RUNS METAGENÓMICA-Proyecto líquidos/qiime/exports/fbcd37c1-7b15-4145-b71f-956bb3073fc6/data/feature-table.tsv", sep = "\t", header = T, skip = 1, comment.char = "")
otutable <- otutable %>% column_to_rownames(var = "X.OTU.ID") %>%
    dplyr::select(-c("X56_Alb.IonXpress_049", "X57_Alb.IonXpress_050", "X63_Alb.IonXpress_054", "X64_Alb.IonXpress_055", "X71_Alb.IonXpress_060", "X77_Alb.IonXpress_066", "X78_Alb.IonXpress_067", "X80_Alb.IonXpress_068"))

sampleData <- readRDS("../RDSs/covs.RDS")
rownames(sampleData) <- sampleData$nombre_muestra

taxonomy <- read.table("../RUNS METAGENÓMICA-Proyecto líquidos/qiime/exports/taxonomy.tsv", sep = "\t", header = T)
taxonomy <- taxonomy %>% column_to_rownames(var = "Feature.ID") %>% dplyr::select(-Confidence)
...
## Cambiar nombres de las columnas de otutable

```{r}
colnames(otutable)
colnames(otutable) <- gsub("_IonXpress_.*", "", colnames(otutable))
colnames(otutable) <- gsub(".IonXpress_.*", "", colnames(otutable))
colnames(otutable) <- gsub("^X", "", colnames(otutable))
colnames(otutable) <- gsub("_IonCode_.*", "", colnames(otutable))
colnames(otutable) <- gsub(".IonCode_.*", "", colnames(otutable))
colnames(otutable) <- gsub("^P([0-9]+)", paste0("DAMPS-P\\1"), colnames(otutable))
colnames(otutable) <- gsub("([0-9])T", "\\1_T", colnames(otutable))
cat("\n")
colnames(otutable)
```
...
otutable_lp <-otutable %>%
    dplyr::select(matches("^[0-9]"), matches("_LP"))
colnames(otutable_lp) <- gsub("_LP", "", colnames(otutable_lp))

setdiff(colnames(otutable_lp), covs$nombre_muestra )

otutable_lp <- otutable_lp %>%
    dplyr::select(-"18_Alb")
```
...
otutable_lp <- otutable_lp[, order(match(colnames(otutable_lp), rownames(sampleData)))]
# coldata rownames match assay colnames
all(rownames(sampleData) == colnames(otutable_lp)) # our dataset
##  [1] TRUE
...
otutable_lp <- otutable_lp[rownames(otutable_lp) %in% rownames(taxaMatrix),]
# rowdata rownames match assay rownames
all(rownames(taxaMatrix) == rownames(otutable_lp))
##  [1] TRUE
...
# Counts
otutable_lp <- as.matrix(otutable_lp)
class(otutable_lp) # should be a numeric matrix
##  [1] "matrix" "array"
```
...
abundances <- data.frame(t(otutable_lp))
...
map_dfr(rarecurve_data, bind_rows) %>%
    ...
    geom_line() +
    theme_minimal()+
    labs(x= "number of species", y = "counts")
...
```{r}
# Create a TreeSE
tse_lp_fastq <- TreeSummarizedExperiment(
...
```{r}
saveRDS(tse_lp_fastq, "../RDSs/tse_lp_fastq.RDS")
```


```{r}
# Realizar PCA en los datos ajustados
pca <- prcomp(t(assay(tse_lp_fastq)))
pca_data <- as.data.frame(pca$x)
pca_data$batch <- colData(tse_lp_fastq)$Batch
